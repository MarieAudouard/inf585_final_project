...
void implicit_surface_structure::update_marching_cube(float isovalue)
{
	...
	if (position.size() > previous_size) {
		...
		drawable_param.shape.material.color = { 0.9,0.9,0.6 }; // to make the texture more yellow
		drawable_param.shape.material.phong.specular = 0.f;
		drawable_param.shape.texture.load_and_initialize_texture_2d_on_gpu(project::path + "assets/sand.jpg",
			GL_REPEAT,
			GL_REPEAT); // I don't understand why it is not possible to see the details of the texture, probably because of the scale of my scene
	}
	else {
...
void implicit_surface_structure::update_field_attributes(float isovalue)
{
	// Variable shortcut
...
void implicit_surface_structure::set_domain(int samples, cgp::vec3 const& length, cgp::vec3 const center)
{
	field_param.domain = spatial_domain_grid_3D::from_center_length(center, length, samples * int3{ 1,1,1 });
}
